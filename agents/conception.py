...
class GuideAgent:
    ...
    async def run(self, query, papers=""): return await llm(f"Query: {query}\nPapers:\n{papers[:2000]}\nGuide.", system=self.SYSTEM, agent="researcher")

class GeneratorAgent:
    SYSTEM = "You are a research idea generator. Produce 5-10 novel ideas with title, hypothesis, methodology, feasibility (1-10)."
    async def run(self, topic, papers=""): return await llm(f"Topic: {topic}\nLiterature:\n{papers[:3000]}\nGenerate ideas.", system=self.SYSTEM, agent="researcher")
...
class IdeaCriticAgent:
    ...
    async def run(self, ideas, papers=""): return await llm(f"Ideas:\n{ideas}\nLiterature:\n{papers[:2000]}\nCritique.", system=self.SYSTEM, agent="critic")

class RankerAgent:
    SYSTEM = "You are a research idea ranker. Rank by expected value: novelty × impact × feasibility. Return top-3."
    async def run(self, ideas_with_critiques): return await llm(f"Ideas+critiques:\n{ideas_with_critiques}\nRank top-3.", system=self.SYSTEM, agent="critic")
...
class StructurerAgent:
    ...
    async def run(self, idea, papers=""): return await llm(f"Idea:\n{idea}\nLiterature:\n{papers[:2000]}\nStructure.", system=self.SYSTEM, agent="researcher")
...
class ConceptionService:
    ...
    async def run_full(self, query, papers="") -> dict:
        ...
        return {"guidance": guidance, "ideas": ideas, "critiques": critiques, "ranked": ranked, "plan": plan}
