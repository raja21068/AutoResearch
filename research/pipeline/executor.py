...
from research.hardware import HardwareProfile, detect_hardware, ensure_torch_available, is_metric_name
...
from research.pipeline.contracts import CONTRACTS, StageContract
...
# ---------------------------------------------------------------------------
from research.pipeline._domain import (  # noqa: E402
    _detect_domain,
    _is_ml_domain,
)
...
# ---------------------------------------------------------------------------
from research.pipeline._helpers import (  # noqa: E402
    StageResult,
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._topic import (  # noqa: E402
    _execute_topic_init,
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._literature import (  # noqa: E402
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._synthesis import (  # noqa: E402
    _execute_synthesis,
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._experiment_design import (  # noqa: E402
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._code_generation import (  # noqa: E402
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._execution import (  # noqa: E402
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._analysis import (  # noqa: E402
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._paper_writing import (  # noqa: E402
...
# ---------------------------------------------------------------------------
from research.pipeline.stage_impls._review_publish import (  # noqa: E402
...
def _run_hitl_pre_stage(
    ...
    from research.hitl.intervention import HumanAction, PauseReason
...
def _run_hitl_post_stage(
    ...
    stage_num = int(stage)
    try:
        from research.hitl.cost_guard import CostGuard
    ...
    if not session.should_pause_after(stage_num):
        ...
        try:
            from research.hitl.smart_pause import SmartPause
...
def _run_collaboration_loop(
    ...
    *,
    ...
    """
    from research.hitl.collaboration import CollaborationSession
    from research.hitl.intervention import HumanAction, PauseReason
        ...
        if (
            ...
            if mc_prm and getattr(mc_prm, "enabled", False):
                ...
                if int(stage) in prm_stages:
                    from research.metaclaw_bridge.prm_gate import ResearchPRMGate
