...
from research.evolution import EvolutionStore, extract_lessons
from research.knowledge.base import write_stage_to_kb
from research.pipeline.executor import StageResult, execute_stage
from research.pipeline.stages import (
    DECISION_ROLLBACK,
    MAX_DECISION_PIVOTS,
    NONCRITICAL_STAGES,
    STAGE_SEQUENCE,
...
def _run_experiment_diagnosis(run_dir: Path, config: RCConfig, run_id: str) -> None:
    ...
    """
    try:
        from research.pipeline.experiment_diagnosis import (
            diagnose_experiment,
        ...
        if not qa.sufficient:
            ...
            from research.pipeline.experiment_repair import build_repair_prompt
...
def _run_experiment_repair(run_dir: Path, config: RCConfig, run_id: str) -> None:
    ...
    """
    try:
        from research.pipeline.experiment_repair import run_repair_loop
        ...
        if repair_result.best_experiment_summary:
            from research.pipeline.experiment_repair import (
...
def execute_pipeline(
    *,
    ...
    event_log = None
    try:
        from research.pipeline.event_log import EventLog, EventType, create_event
    ...
    exp_memory = None
    try:
        from research.memory.experiment_memory import ExperimentMemory
    ...
    for stage in STAGE_SEQUENCE:
        ...
        if stage == Stage.EXPERIMENT_DESIGN and result.status == StageStatus.DONE:
            try:
                from research.pipeline.experiment_spec import ExperimentSpec, MetricDef, generate_spec
        ...
        if stage == Stage.RESULT_ANALYSIS and result.status == StageStatus.DONE:
            try:
                from research.pipeline.experiment_spec import parse_spec, validate_results_against_spec
        ...
        if stage in (Stage.CODE_GENERATION, Stage.EXPERIMENT_RUN) and result.status == StageStatus.DONE:
            try:
                from research.pipeline.pitfall_detector import PitfallDetector
        ...
        if stage in (Stage.EXPERIMENT_RUN, Stage.ITERATIVE_REFINE) and result.status == StageStatus.DONE and exp_memory:
            try:
                from research.memory.experiment_memory import ExperimentOutcome
...
def _package_deliverables(
    ...
    if (
        ...
        try:
            from research.templates import get_template, markdown_to_latex
            from research.pipeline.executor import _extract_paper_title
    ...
    try:
        from research.templates import get_template
    ...
    if tex_path.exists() and bib_path.exists():
        try:
            from research.templates.compiler import compile_latex
...
def _metaclaw_post_pipeline(
    ...
    if l2s and getattr(l2s, "enabled", False) and lessons:
        try:
            from research.metaclaw_bridge.lesson_to_skill import (
    ...
    try:
        from research.metaclaw_bridge.skill_feedback import (
            SkillFeedbackStore,
            record_stage_skills,
        )
        from research.metaclaw_bridge.stage_skill_map import get_stage_config
    ...
    try:
        from research.metaclaw_bridge.session import MetaClawSession
