...
from research.pipeline._domain import _detect_domain
    ...
    *,
    ...
    if _bp_path is not None:
        try:
            ...
            from research.agents.benchmark_agent.orchestrator import BenchmarkPlan
            _bp = BenchmarkPlan(
    ...
    try:
        from research.data import detect_frameworks, load_framework_docs
    ...
    try:
        from research.domains.detector import detect_domain as _dd_s10, is_ml_domain as _is_ml_s10
        _dp = _dd_s10(topic=config.research.topic)
        if not _is_ml_s10(_dp):
            from research.domains.prompt_adapter import get_adapter as _ga
            _adapter = _ga(_dp)
    ...
    if _oc_cfg.enabled:
        from research.pipeline.opencode_bridge import (
            OpenCodeBridge,
            OpenCodeResult,
    ...
    if not _beast_mode_used and config.experiment.code_agent.enabled and llm is not None:
        ...
        from research.pipeline.code_agent import CodeAgent as _CodeAgent
        ...
        if config.experiment.mode in ("sandbox", "docker"):
            from research.experiment.factory import (
        ...
        try:
            from research.domains.detector import detect_domain as _dd
            from research.domains.detector import is_ml_domain as _is_ml
            ...
            if not _is_ml(_domain_profile):
                try:
                    from research.agents.code_searcher import CodeSearchAgent
